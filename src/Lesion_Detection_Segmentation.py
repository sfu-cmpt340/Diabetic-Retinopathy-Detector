...
class LesionDetectionModel():
    ...
    def __init__(self, num_classes, learning_rate=1e-3, device=None):
        ...
        fine_tuned_weights = torch.load('./src/fine_tuned_resnet18_state_dict.pth')
...
class CustomBackboneWithFPN(nn.Module):
    def __init__(self, feature_extractor,model,pretrained=True,trainable_layers=3):
        ...
        self.model =   models.resnet18(weights=None)
        d = torch.load('./src/fine_tuned_resnet18_state_dict.pth')
...
def train_mask_rcnn_epoch(lesion_segmentation_module, loader,test_loader,device,num_epochs):
    ...
    torch.save(lesion_segmentation_module.state_dict(), './src/lesion_segmentation_model_dict.pth')
    # lesion_segmentation_module.eval()  # Set model to evaluation mode
    # s = torch.load('./src/lesion_segmentation_model_dict.pth')
...
def train_lesion_segmentation(num_epochs, optimizer, lesion_segmentation_module, loader,device,test_loader):
    """
